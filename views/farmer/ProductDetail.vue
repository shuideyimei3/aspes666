  ...
  <div class="product-detail-page" v-loading="loading">
    <div class="detail-container" v-if="product">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/farmer' }">农户首页</el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: '/farmer/products' }">我的产品</el-breadcrumb-item>
...
const loadProductDetail = async () => {
  try {
    loading.value = true;
    console.log('Loading product detail for productId:', productId);
...
const toggleProductStatus = async (action) => {
  try {
    ...
    if (action === 'on-sale') {
      await farmerAPI.onSaleProduct(productId);
    } else {
      await farmerAPI.offlineProduct(productId);
    }
